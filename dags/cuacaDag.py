...
def load_data(ds, **kwargs):
	...
	pg_hook  = PostgresHook(postgres_conn_id='cuaca_id')

	file_name = str(datetime.now().date()) + '.json'
	tot_name = os.path.join(os.path.dirname(__file__),'src/data', file_name)
	# tot_name = os.path.join("/opt/airflow/data", file_name)
	...
	insert_cmd = """INSERT INTO cuaca_table 
...
dag = DAG(
		...
		description='A simple DAG to fetch weather data using OpenWeatherMap API',
		catchup=False,  # 🔥 Penting: tidak akan backfill ke tanggal lama
		start_date=datetime(2025,6,14),
		schedule_interval=timedelta(minutes=1440))


# First task is to query get the weather from openweathermap.org.
#bash_command='cd /home/ubaid/airflow/dags/src && python3 getWeather.py',
task1 = BashOperator(
			task_id='get_weather',
			bash_command='python3 /opt/airflow/dags/src/getWeather.py',
			dag=dag)
